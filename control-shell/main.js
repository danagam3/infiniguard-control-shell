const { app, BrowserWindow, globalShortcut, ipcMain, screen, Menu, shell, session } = require('electron');
...
let loadedDay = new Date().toDateString();
function freshURL(){ return APP_URL + '?v=' + Date.now(); }   // מונע טעינת גרסה שמורה ישנה
...
app.whenReady().then(() => {
  ...
  try { session.defaultSession.clearCache(); } catch (e) {}   // ניקוי מטמון בכל הפעלה
  createWindow();
  // רענון אוטומטי כשמתחלף היום — התאריך תמיד עדכני + נטענת הגרסה החדשה מ-Netlify
  setInterval(() => {
    const today = new Date().toDateString();
    if (today !== loadedDay) {
      loadedDay = today;
      if (win && !win.isDestroyed()) { try { win.loadURL(freshURL()); } catch (e) {} }
    }
  }, 5 * 60 * 1000);
